...
class DerivationOfSumFormula(Scene):
    def construct(self):
        ...
        derivativeEqualZeroPtSix = MathTex(r"r \approx 37 \% * ({n-1})")

        limitThingOne = MathTex(r"\lim_{n \to \infty} {n-1} = \lim_{n \to \infty} {n}")
        limitThingTwo = MathTex(r"\therefore \lim_{n \to \infty} \frac{r}{n} = \frac {1}{e}")
        ...
        self.wait(1)
        self.play(Transform(naive, derivativeEqualZeroPtSix))
        self.wait(1)
        self.play(Transform(naive, limitThingOne))
        self.wait(1)
        self.play(Transform(naive, limitThingTwo))

        subbingIntoProbability = MathTex(r"\frac{\frac{n-1}{e}}{n}")
